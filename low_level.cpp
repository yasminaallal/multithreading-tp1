...
int main() {
  ...
  if (get_response.status_code != 200) {
    std::cerr << "Échec de la requête GET, code : " << get_response.status_code
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  try {
    ...
    auto matrix_values =
        received_data.at("a").get<std::vector<std::vector<double>>>();
    ...
    if (num_rows == 0 || matrix_values[0].size() != num_cols) {
      std::cerr
          << "Erreur : dimensions incohérentes entre la matrice et le vecteur."
          << std::endl;
      return EXIT_FAILURE;
    }
    ...
    Eigen::VectorXd solution_vector =
        coefficient_matrix.llt().solve(right_hand_side);
    auto end_time = std::chrono::high_resolution_clock::now();

    double execution_time =
        std::chrono::duration<double>(end_time - start_time).count();

    std::cout << "Tâche " << task_id << " complétée en " << execution_time
              << " secondes." << std::endl;

    // Création de la réponse JSON
    nlohmann::json output_json = {{"a", matrix_values},
                                  {"b", vector_values},
                                  {"x", solution_vector},
                                  {"time", execution_time},
                                  {"identifier", task_id}};
    ...
    cpr::Response post_response = cpr::Post(
        cpr::Url{server_url}, cpr::Header{{"Content-Type", "application/json"}},
